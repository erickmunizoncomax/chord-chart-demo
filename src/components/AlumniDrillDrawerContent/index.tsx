...
import { CloseButtonContainer } from '../PostDocDrillDrawerContent'

interface AlumniDrillDrawerContentProps {
  opened: boolean
...
const getEchartsDatasetRows = (
  ...
  const years = Array.from(new Set(data.map(d => d.OUTCOME_YEAR))).sort()

  const counts: Record<string, Record<string, number>> = {}
  for (const item of data) {
    const year = item.OUTCOME_YEAR as string
...
const AlumniDrillDrawerContent: FC<AlumniDrillDrawerContentProps> = (props) => {
  const {
    // data,
    drillDownFieldValue,
    dimensionField,
  ...
  const chartOptions = useMemo(() => {
    ...
    const filtered = data.filter(item => {
      return (
        ...
        (
          !broadtype ||
          item.broadtype === broadtype
        )*/
      )
    })

    const seriesNames = getSeriesNames(filtered, dimensionField)
    const rows = getEchartsDatasetRows(filtered, seriesNames, dimensionField)
            ...
            <div className="kpi-info-container flex items-center justify-between bg-[#111928] px-12 h-40">
              ...
              <div className="right-content">
                <div className="kpi-gender-container flex items-center justify-between">
                  <KpiGender Icon={Woman} label="Female" value="44%" />
                  <KpiGender Icon={Man} label="Male" value="55%" />
                </div>
              </div>
            </div>

            <div className="chart-container bg-[#111928] w-full h-full relative max-h-[calc(100%-17rem)] px-12 pb-12">
              <div className="component-container p-4 border border-[#FFFFFF20] rounded-2xl h-full flex flex-col justify-between">
...
export default AlumniDrillDrawerContent
